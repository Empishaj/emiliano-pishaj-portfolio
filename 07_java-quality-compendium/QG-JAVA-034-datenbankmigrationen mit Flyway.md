...
| --- | --- |
| Dokumenttyp | Java Quality Guideline / Persistenz- und DevOps-Standard |
| ID | QG-JAVA-034|
| Titel | Datenbankmigrationen mit Flyway |
| Status | Accepted / verbindlicher Standard für DB-Schema- und Datenmigrationen in Java-/Spring-Boot-Services |
| Version | 2.0 |
| Datum | 2026-05-18|
| Java-Baseline | Java 21 |
| Framework-Baseline | Spring Boot 3.4+, Flyway 10.x (Spring Boot 3.4 Default) bzw. Flyway 11.x für neueste Features |
| Datenbank-Baseline | PostgreSQL 16+ (Default — andere Datenbanken siehe Sektion 28) |
| Test-Baseline | Testcontainers 1.20+ mit `postgres:16-alpine` Image |
| Kategorie | Persistenz · DevOps · Release Engineering · Database Operations |
| Zielgruppe | Java-Entwickler, Tech Leads, DBAs, DevOps, Platform-Team, Reviewer, QA, Security |
| Verbindlichkeit | Neue Spring-Boot-Services MÜSSEN Flyway für Schema-Management verwenden. `ddl-auto=create`/`create-drop`/`update` sind in Produktion VERBOTEN. Abweichungen sind im Pull Request nachvollziehbar zu begründen. |
| Technische Validierung | Code-Beispiele sind gegen Flyway 10/11, Spring Boot 3.4 und PostgreSQL 16 referenzbasiert validiert. Lock-Patterns sind gegen PostgreSQL 16 getestet. Java-Migration-Beispiele kompilieren gegen Flyway 10 API. |
| Schwester-Guidelines | QG-JAVA-006 v2 (Spring-Boot-Serviceschicht, Tenant-Context), QG-JAVA-008 v2 (Objektorientierung, JPA-Entities), QG-JAVA-019 v2 (Contract Testing), QG-JAVA-041 v2 (Event-Driven Architecture, Outbox-Pattern), QG-JAVA-126 v2 (Docker, Container-Migrationen) |
| Wesentliche Änderungen ggü. v1.0 | Flyway 10+/11+ statt 9+ · `baseline-on-migrate: true` als Anti-Pattern kennzeichnet · Repeatable Migrations mit Idempotenz-Pflicht erklärt · Multi-Tenant- und SaaS-Aspekte komplett ergänzt (Schema-pro-Tenant, programmatische Migration) · PostgreSQL-11+ `ADD COLUMN NOT NULL DEFAULT` Verhalten richtiggestellt · `NOT VALID` / `VALIDATE CONSTRAINT` für non-blocking CHECK/FK Patterns · Lock-Timeout und Statement-Timeout für Migrations · Datenbackfills über Application Layer statt Migration-UPDATE · Java-Migration (`BaseJavaMigration`) als Alternative · `CREATE INDEX CONCURRENTLY` mit `transactional: false` · Realistische Rollback-Strategie (Forward-Fix als Standard) · `out-of-order` Trade-off und Timestamp-basierte Versionierung · Repeatable-Sortierung mit numerischem Präfix · Kubernetes Init-Container-Pattern · Separate DB-User für Migration vs. App · Schema- vs. Daten-Migrationen explizit getrennt · Cross-References zu QG-JAVA-006 v2 (Tenant-Context), QG-JAVA-041 v2 (Outbox-Pattern) · strukturell an QG-JAVA-006 v2 angeglichen (TOC, Lese-Anleitung, MUSS/DARF/SOLLTE getrennt, Begriffsglossar als eigene Sektion, RFC-2119-Großschreibung) · ID-Konvention an Compendium-Reihe angepasst (QG-JAVA-NNN) |
| Nicht-Ziel | Diese Richtlinie ersetzt keine vollständige DBA-Schulung, keine PostgreSQL-Performance-Tuning-Doku und keine Datenbank-Cluster-Operations-Anleitung. Sie behandelt keine NoSQL-Migrations (MongoDB, Cassandra), keine Big-Data-Pipelines und keine Event-Sourcing-Schema-Evolution (siehe QG-JAVA-041 v2 für Letzteres). |
...
6. [Technischer Hintergrund: Flyway 10+ und PostgreSQL 16+](#6-technischer-hintergrund-flyway-10-und-postgresql-16)
7. [Niemals `ddl-auto=create` in Produktion](#7-niemals-ddl-autocreate-in-produktion)
8. [Flyway-Namenskonvention und Versionsschema](#8-flyway-namenskonvention-und-versionsschema)
9. [Migrationen sind unveränderlich](#9-migrationen-sind-unveränderlich)
10. [Schema-Migrationen vs. Daten-Migrationen](#10-schema-migrationen-vs-daten-migrationen)
11. [Repeatable Migrations und Idempotenz-Pflicht](#11-repeatable-migrations-und-idempotenz-pflicht)
12. [`baseline-on-migrate` als Anti-Pattern](#12-baseline-on-migrate-als-anti-pattern)
13. [Gute Schema-Migrationsskripte](#13-gute-schema-migrationsskripte)
14. [Zero-Downtime-Migrationen mit PostgreSQL 16+](#14-zero-downtime-migrationen-mit-postgresql-16)
15. [`NOT VALID` und `VALIDATE CONSTRAINT` für non-blocking Constraints](#15-not-valid-und-validate-constraint-für-non-blocking-constraints)
16. [Lock-Timeouts und Statement-Timeouts](#16-lock-timeouts-und-statement-timeouts)
17. [`CREATE INDEX CONCURRENTLY` und `transactional: false`](#17-create-index-concurrently-und-transactional-false)
18. [Datenbackfills über Application Layer](#18-datenbackfills-über-application-layer)
19. [Java-Migrationen mit `BaseJavaMigration`](#19-java-migrationen-mit-basejavamigration)
20. [Multi-Tenant: Schema-pro-Tenant und programmatische Migration](#20-multi-tenant-schema-pro-tenant-und-programmatische-migration)
21. [Spring-Boot-Konfiguration](#21-spring-boot-konfiguration)
22. [Kubernetes-Init-Container-Pattern](#22-kubernetes-init-container-pattern)
23. [Separate DB-User für Migration und App](#23-separate-db-user-für-migration-und-app)
24. [Rollback-Strategie: Forward-Fix als Standard](#24-rollback-strategie-forward-fix-als-standard)
25. [`out-of-order` und Timestamp-basierte Versionierung](#25-out-of-order-und-timestamp-basierte-versionierung)
26. [Testumgebung: Flyway + Testcontainers](#26-testumgebung-flyway--testcontainers)
27. [CI/CD-Integration](#27-cicd-integration)
28. [Datenbankspezifische Aspekte](#28-datenbankspezifische-aspekte)
29. [Anti-Patterns](#29-anti-patterns)
30. [Migration bestehender Setups](#30-migration-bestehender-setups)
31. [Review-Checkliste](#31-review-checkliste)
32. [Automatisierbare Prüfungen](#32-automatisierbare-prüfungen)
33. [Ausnahmen](#33-ausnahmen)
34. [Definition of Done](#34-definition-of-done)
35. [Quellen und weiterführende Literatur](#35-quellen-und-weiterführende-literatur)

### Wie liest du dieses Dokument

Dieses Dokument ist mit ca. 100 KB ein Nachschlagewerk, kein Lesebuch. Vier Lesepfade werden empfohlen:

**Wenn du neu im Team bist:** Starte mit Sektion 2 (Kurzregel), lies dann Sektion 7 (Kein `ddl-auto=create`) und Sektion 9 (Unveränderlichkeit). Damit hast du die mentale Karte für 80 % aller Migration-Reviews. Danach Sektion 13 (Gute Skripte) und Sektion 14 (Zero-Downtime) — die zwei härtesten Production-Lehren.

**Wenn du im Code-Review bist:** Springe direkt zu Sektion 31 (Review-Checkliste). Jede Zeile hat einen Anker zur Detail-Sektion mit der Begründung. Bei Verstoß gegen eine MUSS-Regel: Sektion 3.1.

**Wenn du etwas Spezifisches suchst:** Inhaltsverzeichnis oben. Häufigste Punkte: Anti-Patterns → Sektion 29. Multi-Tenant → Sektion 20. Lock-Timeouts → Sektion 16. Backfills → Sektion 18. Kubernetes → Sektion 22.

**Wenn du eine Production-Migration planst:** Sektion 14 (Zero-Downtime), dann Sektion 15 (`NOT VALID`), dann Sektion 16 (Lock-Timeouts), dann Sektion 18 (Backfill-Strategie). Bei Multi-Tenant zusätzlich Sektion 20. Vor Deployment Sektion 24 (Rollback-Strategie) durchgehen.
